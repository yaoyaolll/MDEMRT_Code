...
{
	EALLOW;
	GpioMuxRegs.GPFMUX.all = 0xFFFF;    //  0011 1111 0000 0000 ,设置使能MCBSP引脚同时其他未设置的引脚默认为IO.
	EDIS;

//	开启MCBSP接受与发送中断
	PieCtrlRegs.PIEIER6.bit.INTx5 = 1;  //开启MCBSP接收中断
...
//	McbspRegs.MFFINT.bit.XINT = 1;  //非FIFO模式下使能发送中断
	...
	asm(" NOP");
    EALLOW;
	PieVectTable.MRINTA = &MRINTA_ISR1;
	EDIS;
